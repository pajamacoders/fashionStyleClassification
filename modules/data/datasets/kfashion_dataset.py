...
class KfashionDataSet(Dataset):
	def __init__(self, cfg, split):
		self.image_root = f'/data/kfashion_images/{split}/images'
		self.labels_root = os.path.join(cfg.DATA.BASE_PATH, cfg.DATA.DATASET)

		self.fnamelist = []
		filepath = os.path.join(self.labels_root, cfg.DATA.PATH_FILE[split.upper()])
	...
	def __getitem__(self, index):
		path = os.path.join(self.image_root, self.fnamelist[index[0]])
